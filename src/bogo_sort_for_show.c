...
void bogosort(void *list, size_t nitems, size_t size, int (*compr)(const void*, const void*)) {
	size_t trialsleft = 13;
	while (!is_sorted(list, nitems, size, compr) && trialsleft--) {
		shuffle(list, nitems, size);
	}
}

int main(int argc, char** argv) {
	return perform_sort_visual_args(argc, argv, bogosort, "bogo sort");
}
