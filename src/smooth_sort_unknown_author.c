 ...
 *
 * In this project, Snqzs' PG had made a modification to the implementation found on the Internet:
 *     To eliminate the need of a pre-defined array of Leonardo's numbers. (This negatively impacted the performance)
 *
...
/**
 * Adopted implementation from https://stackoverflow.com/questions/1390832/how-to-sort-nearly-sorted-array-in-the-fastest-time-possible-java/28352545#28352545
 */
...
static void shift_down(pshift_t *pshift, size_t byn) {
	...
	// }
	// if (pshift -> lpsm2 > pshift -> lpsm1) {
	// 	printf("ini_pshift = %lu\n", (unsigned long) ini_pshift);
	// 	printf("byn = %lu\n", (unsigned long) byn);
	// 	printf("pshift -> lpsm2 = %lu\n", (unsigned long) pshift -> lpsm2);
	// }
...
static void sift(void *list, size_t nitems, size_t size, pshift_t pshift, size_t head, int (*compr)(const void*, const void*)) {
	...
	while (pshift.pshift > 1) {
		...
		if (compr(val, list + lf * size) >= 0 && compr(val, list + rt * size) >= 0) {
			break;
		}
		if (compr(list + lf * size, list + rt * size) >= 0) {
			if (head != lf) {
...
int main(int argc, char** argv) {
	return vis_main_comparison(argc, argv, "smooth sort (unknown author's implementation)", ssort);
}
